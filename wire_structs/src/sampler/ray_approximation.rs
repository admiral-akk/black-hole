...
impl RayApproximation {
    ...
    pub fn new(final_angle: f32, curve_dist: f32, start_dist: f32) -> Self {
        Self {
            curve_dist,
            final_angle,
            start_dist,
    ...
    pub fn generate_optimal(ray: &SimulatedRay, start_dist: f32, angle: &DimensionParams) -> Self {
        let mut dist_bounds = [0., start_dist];
        ...
        while dist_bounds[1] - dist_bounds[0] > 0.0001 {
            ...
            let r_1 = RayApproximation::new(final_angle, d_1, start_dist);
            let r_2 = RayApproximation::new(final_angle, d_2, start_dist);
        ...
        RayApproximation::new(
            final_angle,
            (dist_bounds[0] + dist_bounds[1]) / 2.,
            start_dist,
        )
    }

    pub fn get_dist(&self, angle: f32) -> f32 {
        let theta_1 = f32::acos(self.curve_dist / self.start_dist);
        let theta_2 = self.final_angle - FRAC_PI_2;
        if angle < theta_1 {
            let angle = theta_1 - angle;
            return self.curve_dist / angle.cos();
        } else if angle < theta_2 {
            return self.curve_dist;
        } else {
...
mod test {
    ...
    #[test]
    fn simple_approx() {
        let approx = RayApproximation::new(PI, 5., 10.);

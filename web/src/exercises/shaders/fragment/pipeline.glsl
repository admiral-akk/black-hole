...
vec3 get_color(vec2 coord){
    ...
    float max_z=texture(z_max_cache,vec2((distance-5.)/15.,.0)+.5/vec2(z_max_cache_dim)).x;
    ...
    if(start_dir.z<max_z){
        ...
        vec3 cached_dir=texture(direction_cache,vec2(index,(distance-5.)/15.)+.5/vec2(cache_dim)).xzy;
        float angle=PI/2.;
        if(start_dir.x!=0.){

...
mod tests {
    ...
    fn find_near_miss(camera_pos: &DVec3, field: &Field, max_width: f64) -> (DVec3, DVec3) {
        ...
        while (left - right).length() > max_width {
            ...
            if cast_ray_steps(&ray, field, 20.0, 100.0).1.is_none() {
                // hit the black hole
                right = center;
            } else {
                left = center;
    ...
    fn plot_fixed_radius() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for radius in 1..=10 {
            ...
            let field = Field::new(r, start.length());
            ...
            for i in 0..num_lines {
                ...
                let (path, hit) = cast_ray_steps(&ray, &field, 20.0, 100.0);
                is_hit.push(hit.is_none());
                lines.push(path);
            }
